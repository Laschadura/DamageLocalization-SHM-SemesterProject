...
p.addOptional('eps_cluster',0.2); %original 0.2
p.parse(varargin{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % t: time vector associated with the IRF
    %
    ...
    if size(x,1) > size(x,2)
        x = x';  % Ensure x is in the correct orientation
        [Nxx, ~] = size(x);
    else
        [Nxx, ~] = size(x);
    end

    % Safeguard M calculation to avoid exceeding available data length
    M = min(round(Ts / dt), size(x, 2));
    
    % Debugging: Check the value of M
    disp('M value:');
    disp(M);
        ...
        case 1
            ...
            for oo = 1:Nxx
                for jj = 1:Nxx
                    ...
                    y2 = fft(x(jj, :));
                    h0 = ifft(y1 .* conj(y2));
                    % Debugging: Check the size of h0
                    disp('Size of h0:');
                    disp(size(h0));
                    IRF(oo, jj, :) = h0(1:M);
                end
            end
            % Get time vector t associated with the IRF
            t = (0:1:M-1) * dt;
        ...
        case 2
            ...
            for oo = 1:Nxx
                for jj = 1:Nxx
                    ...
                    % Debugging: Check the size of dummy
                    disp('Size of dummy:');
                    disp(size(dummy));
