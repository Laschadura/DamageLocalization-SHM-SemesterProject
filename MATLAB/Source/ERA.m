function [yx,freq_rel2,damp_rel,modal_shapes,freq_rel,ao,bo,co] = ERA(yx,dt,refNum,begin,len,ndof,order)
...
% % preview the processed part of the signal
% figure
% ax(1) = subplot(3,1,1);
% plot(yx)
% title('Input data')
...
% ax(2) = subplot(3,1,2);
% plot(yx)
% title('Signals normalized to the reference channel')
% xlabel({'Sample #'});
% ylabel({'Normalized [1]'});
...
% calculate matrix ao (eq 10.136 - "MA" , where Ep and El are assumed to be diag(1))
ao=L*h2*R;
...
% just for DEBUG
% build a discrete time system out of identified data
% identif_c = ss(ao,bo,co,0,dt)
% convert discrete system to continues system
